          ...
          session[:results]['SearchRequestGet']['SearchCriteriaWithActions']['LimitersWithAction'].each do |appliedLimiter|
            ...
            if appliedLimiter["Id"] == "DT1"
              appliedLimiter["LimiterValuesWithAction"].each do |limiterValues|
                appliedlimiters << '<span class="btn-group appliedFilter constraint filter filter-' + appliedLimiter["Id"] + '"><a class="constraint-value btn btn-sm btn-default btn-disabled" href="' + request.fullpath.split("?")[0] + "?" + generate_next_url + "&eds_action=" + appliedLimiter["RemoveAction"].to_s + '"><span class="filterName">' + limiterLabel.to_s.titleize + '</span><span class="filterValue">' + limiterValues["Value"].gsub("-01/"," to ").gsub("-12","") + '</span></a><a class="btn btn-default btn-sm remove dropdown-toggle" href="' + request.fullpath.split("?")[0] + "?" + generate_next_url + "&eds_action=" + appliedLimiter["RemoveAction"].to_s + '"><span class="glyphicon glyphicon-remove"></span><span class="sr-only">Remove limiter '+ limiterLabel.to_s.titleize +':' + limiterValues["Value"].gsub("-01/"," to ").gsub("-12","") + '</a></span>'
              end
            else
